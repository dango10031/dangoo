...
// pb_hooks/wan22.pb.js — RunningHub AI 应用中转模板
//
...
routerAdd("GET", "/api/aigc/ai-app/wan22/schema", function (e) {
  // SAFE_SCHEMA 只含前端渲染表单需要的安全字段, 由 install_ai_app 从 manifest 注入。
  // 绝不含 nodeId / fieldName / webappId / RunningHub 端点。
  var SAFE_SCHEMA = {"version": "rh-ai-app.v1", "slug": "wan22", "model": "ai-app-wan22", "title": "WAN 2.2 图生视频", "intent": "image-to-video", "fields": [{"key": "referenceImage", "label": "图片", "type": "image", "required": false, "group": "basic", "defaultValue": "bfba92d838e38533b50b3778fb2c263a5411b30f50b5aa700d6058c16061d011.png", "description": "图片"}, {"key": "text", "label": "提示词", "type": "textarea", "required": false, "group": "basic", "defaultValue": "图中的小男孩，向外拉扯浅蓝色儿童平角内裤的黄色腰头，而后稍稍收回，又继续拉扯腰头展示面料拉伸弹力；身穿纯白色短袖 T 恤，赤脚，纯白无影摄影棚背景，柔和均匀棚拍柔光，写实儿童内衣商业实拍，面料网眼纹理清晰，男孩面带自然微笑，人体结构完整无畸变，画面稳定无抖动，镜头缓缓推进至腰部，特写", "description": "提示词"}, {"key": "node51_value", "label": "最长边", "type": "number", "required": false, "group": "basic", "defaultValue": "1280", "description": "最长边"}], "outputs": {"expectedTypes": ["image"], "render": "gallery"}}

  return e.json(200, SAFE_SCHEMA)
})

routerAdd("POST", "/api/aigc/ai-app/wan22/upload", function (e) {
  function readKey(ev) {
  ...
  try {
    ...
    if (!res || res.statusCode < 200 || res.statusCode >= 300) {
      try { $app.logger().error("wan22_upload_http_error status=" + String(res ? res.statusCode : 0) + " body_length=" + String(raw.length)) } catch (_) {}
      return e.json(502, { error: "rh_upload_failed", message: "文件上传失败, 请稍后重试" })
    }
    var parsed = responseJson(res)
    var fileName = parsed && (parsed.fileName || (parsed.data && (parsed.data.fileName || parsed.data.filename)))
    if (!fileName) {
      try { $app.logger().error("wan22_upload_bad_response body_length=" + String(raw.length)) } catch (_) {}
      return e.json(502, { error: "rh_upload_bad_response", message: "文件上传失败, 请稍后重试" })
    }
    ...
    try { $app.logger().error("wan22_upload_error: " + msg) } catch (_) {}
    return e.json(500, { error: "ai_app_upload_error", message: "文件上传失败, 请稍后重试" })
  }
})

routerAdd("POST", "/api/aigc/ai-app/wan22/run", function (e) {
  var VIBEX_APP_ID = "app-ca81449e917d4660aa213c5c13d47008"
  var WEBAPP_ID = "2070029147514163202"
  var MODEL = "ai-app-wan22"
  var NODE_INFO_TEMPLATE = [{"nodeId": "73", "fieldName": "image", "fieldValue": "bfba92d838e38533b50b3778fb2c263a5411b30f50b5aa700d6058c16061d011.png"}, {"nodeId": "72", "fieldName": "text", "fieldValue": "图中的小男孩，向外拉扯浅蓝色儿童平角内裤的黄色腰头，而后稍稍收回，又继续拉扯腰头展示面料拉伸弹力；身穿纯白色短袖 T 恤，赤脚，纯白无影摄影棚背景，柔和均匀棚拍柔光，写实儿童内衣商业实拍，面料网眼纹理清晰，男孩面带自然微笑，人体结构完整无畸变，画面稳定无抖动，镜头缓缓推进至腰部，特写"}, {"nodeId": "51", "fieldName": "value", "fieldValue": "1280"}]
  ...
  // 用户身份 = 站内登录账号(经全局守卫注入)。出站密钥只取服务端环境变量,
  // 不是 JWT。不要用 JWT decode 取 sub —— 代理不传 JWT, 会让 rh_user_id 恒为空串。
  ...
  function buildNodeInfoList(body) {
    ...
    var __v0 = (body["referenceImage"] === undefined || body["referenceImage"] === null) ? "bfba92d838e38533b50b3778fb2c263a5411b30f50b5aa700d6058c16061d011.png" : body["referenceImage"]
    setNodeValue(nodes, "73", "image", (__v0 === undefined || __v0 === null) ? "" : String(__v0))
    var __v1 = (body["text"] === undefined || body["text"] === null) ? "图中的小男孩，向外拉扯浅蓝色儿童平角内裤的黄色腰头，而后稍稍收回，又继续拉扯腰头展示面料拉伸弹力；身穿纯白色短袖 T 恤，赤脚，纯白无影摄影棚背景，柔和均匀棚拍柔光，写实儿童内衣商业实拍，面料网眼纹理清晰，男孩面带自然微笑，人体结构完整无畸变，画面稳定无抖动，镜头缓缓推进至腰部，特写" : body["text"]
    setNodeValue(nodes, "72", "text", (__v1 === undefined || __v1 === null) ? "" : String(__v1))
    var __v2 = (body["node51_value"] === undefined || body["node51_value"] === null) ? "1280" : body["node51_value"]
    setNodeValue(nodes, "51", "value", (__v2 === undefined || __v2 === null) ? "" : String(__v2))
    return nodes
  }
  ...
  try {
    ...
    if (!res || res.statusCode < 200 || res.statusCode >= 300) {
      try { $app.logger().error("wan22_run_http_error status=" + String(res ? res.statusCode : 0) + " body=" + String(raw).substring(0, 500)) } catch (_) {}
      // 上游 5xx/网关错误是瞬时故障, 明确告知可直接重试(提交在受理前失败, 未扣费)
      var upStatus = res ? Number(res.statusCode) : 0
      var transient = upStatus === 0 || upStatus === 429 || upStatus >= 500
      return e.json(transient ? 503 : 502, {
        error: "rh_ai_app_run_http_error",
        message: transient ? "生成服务暂时繁忙, 请稍后点「单项重试」(本次未扣费)" : "任务提交失败, 请稍后重试",
        retryable: transient,
      })
    }
    var parsed = responseJson(res)
    if (!parsed) {
      try { $app.logger().error("wan22_run_bad_json body_length=" + String(raw.length)) } catch (_) {}
      return e.json(502, { error: "rh_ai_app_run_bad_json", message: "任务提交失败, 请稍后重试" })
    }
    ...
    if (nodeErrors) return e.json(400, { ok: false, version: "rh-ai-app.v1", state: "failed", error: { code: "RH_NODE_ERRORS", message: "输入参数未通过 RunningHub 校验", retryable: false, taskId: "" } })
    ...
    if (!taskId) {
      try { $app.logger().error("wan22_run_bad_response body_length=" + String(raw.length)) } catch (_) {}
      return e.json(502, { error: "rh_ai_app_run_bad_response", message: "任务提交失败, 请稍后重试" })
    }
    var jobId = newJobId()
    try {
      var coll = ensureColl()
    ...
    if (msg.indexOf("缺少必填项") === 0 && typeof body !== "undefined" && body && typeof body.inputs === "object" && body.inputs !== null) {
      return e.json(400, {
        ...
        message: "提交参数不能包在 inputs 对象里, 必须平铺在请求体顶层(字段名与 GET /schema 返回的 fields[].key 一致)。",
        fingerprint: "ai_app_run_inputs_wrapper",
      })
    }
    try { $app.logger().error("wan22_run_error: " + msg) } catch (_) {}
    return e.json(500, { error: "ai_app_run_error", message: "任务提交失败, 请稍后重试" })
  }
})
...
routerAdd("POST", "/api/aigc/ai-app/wan22/price-preview", function (e) {
  var WEBAPP_ID = "2070029147514163202"
  var NODE_INFO_TEMPLATE = [{"nodeId": "73", "fieldName": "image", "fieldValue": "bfba92d838e38533b50b3778fb2c263a5411b30f50b5aa700d6058c16061d011.png"}, {"nodeId": "72", "fieldName": "text", "fieldValue": "图中的小男孩，向外拉扯浅蓝色儿童平角内裤的黄色腰头，而后稍稍收回，又继续拉扯腰头展示面料拉伸弹力；身穿纯白色短袖 T 恤，赤脚，纯白无影摄影棚背景，柔和均匀棚拍柔光，写实儿童内衣商业实拍，面料网眼纹理清晰，男孩面带自然微笑，人体结构完整无畸变，画面稳定无抖动，镜头缓缓推进至腰部，特写"}, {"nodeId": "51", "fieldName": "value", "fieldValue": "1280"}]
...
routerAdd("POST", "/api/aigc/ai-app/wan22/jobs/{jobId}/poll", function (e) {
  var MODEL = "ai-app-wan22"
  ...
  try {
    ...
    if (status === "FAILED" || status === "CANCEL") {
      var safeError = "task_failed"
      try {
        rec.set("status", "failed"); rec.set("error_message", safeError)
        rec.set("consume_money", usage.consumeMoney || ""); rec.set("consume_coins", usage.consumeCoins || "")
        rec.set("task_cost_time", usage.taskCostTime || ""); rec.set("third_party_consume_money", usage.thirdPartyConsumeMoney || "")
        $app.save(rec)
      } catch (_) {}
      reportTaskIndex({ task_id: jobId, rh_task_id: taskId, rh_user_id: rec.getString("rh_user_id"), status: "failed", error_message: safeError })
      settleFailure(rec)
      return e.json(200, { ok: false, version: "rh-ai-app.v1", state: "failed", job: { jobId: jobId, taskId: taskId, state: "failed" }, outputs: [], results: [], error: { code: "RH_AI_APP_FAILED", message: "生成失败, 请稍后重试", retryable: false, taskId: taskId }, usage: usage })
    }
    return e.json(200, { ok: true, version: "rh-ai-app.v1", state: "running", job: { jobId: jobId, taskId: taskId, state: "running" }, outputs: [], results: [] })
  } catch (err) {
    var msg = String(err && err.message || err)
    if (isRhAuthErr(msg)) return e.json(412, { error: "rh_login_required", message: "RunningHub 登录态已过期, 请用右上角按钮重新登录" })
...
routerAdd("POST", "/api/aigc/ai-app/wan22/history", function (e) {
  var MODEL = "ai-app-wan22"
  ...
  try {
    ...
    var msg = String(err && err.message || err)
    try { $app.logger().error("wan22_history_error: " + msg) } catch (_) {}
    return e.json(500, { error: "ai_app_history_error", message: "历史记录加载失败, 请稍后重试" })
  }
})

routerAdd("POST", "/api/aigc/ai-app/wan22/history/{jobId}/update", function (e) {
  var MODEL = "ai-app-wan22"
  ...
  try {
    ...
    var msg = String(err && err.message || err)
    try { $app.logger().error("wan22_history_update_error: " + msg) } catch (_) {}
    return e.json(500, { error: "ai_app_history_update_error", message: "历史记录更新失败, 请稍后重试" })
  }
})

routerAdd("POST", "/api/aigc/ai-app/wan22/history/{jobId}/delete", function (e) {
  var MODEL = "ai-app-wan22"
  ...
  try {
    ...
    var msg = String(err && err.message || err)
    try { $app.logger().error("wan22_history_delete_error: " + msg) } catch (_) {}
    return e.json(500, { error: "ai_app_history_delete_error", message: "历史记录删除失败, 请稍后重试" })
  }
})
