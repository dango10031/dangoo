...
function rhSourcePlugin() {
  return {
    ...
    transform(code: string, id: string) {
      ...
      traverse(ast, {
        JSXOpeningElement(p: { node: SourceElement }) {
          const node = p.node
          const loc = node.loc
          if (!loc) return
      const exists = node.attributes.some((a) => {
        const attributeName = a.type === 'JSXAttribute' ? a.name : null
        const attributeNameText =
          typeof attributeName === 'string'
            ? attributeName
            : (attributeName as { name?: unknown } | undefined)?.name
        return attributeNameText === 'data-rh-src'
      })
          if (exists) return
